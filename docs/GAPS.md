...
| G-SEAM-18 | **A caller-supplied `maxDepth` LIFTS the deployment depth cap; it is not an enforced ceiling.** | OPEN — security-relevant finding | `resolveChildDepth(parent, request.maxDepth)` (`packages/subagent/subagent/src/child-agent.ts:50`) treats the request value as an *absolute cap the child must not exceed*, so a LARGER number admits a deeper delegation. `SubagentRuntime.resolveMaxDepth(configured)` (`src/index.ts:248`) gives a **tool-row-configured** value precedence over the user setting, so a preset row can widen what the deployment intended. Measured: a grandchild was ADMITTED when the caller passed `maxDepth: 99` while the runtime was configured with `maxDepth: 1`. **This project is unaffected through its own path** — `launch-port.ts` passes `deps.maxDepth` from deployment config and never from the model — but any direct `startContinuable` caller can widen it. Recorded rather than worked around. |
