...
## 8.5 一个必须记录的时序事实：推送先于门（已修正）

我派了 5 个 agent 做推送前审计（凭证 / 体积 / 可推送性 / 推送 / 独立验证），**P4 被要求等三个 GO 门**。但用户随后指示**立即推送**（目的是拿给外部审计）。我推了，**然后在门交回时发现 P3 的判定是 `NO-GO`**。

**P3 是对的，它抓到了三个我漏掉的真缺陷，而它们已经发布在公开仓库里：**

| # | 缺陷 | 后果 |
|---|---|---|
| 1 | `s6-ipy15-before.ts:34` 与 `s6-ipy15-measure.ts:39` 的 `S6_EVIDENCE_DIR` **默认值指向另一个写入者的临时工作树**（`D:/DSH/work/wt-s6/...`） | 有 env 覆盖所以正确调用可用，但**默认值**会让裸跑写进调用者不拥有的树 —— 正是 `cross-tree-paths.test.ts` 拒绝的类 |
| 2 | `packages/dsh-ipython/cordis.patch.yml:42` 硬编码 `C:/Users/hzq00/.../Python314/python.exe`，**无任何覆盖** | 新克隆**能构建、能启动，但无法启动 kernel** —— 在任何其他机器上不可用 |
| 3 | `M-DEP-SEC-UPG/declared-vs-imported.txt` 在 HEAD 说 imported (21) / declared (10) 且漏掉 `dsh-commands`，而同一 HEAD 的 `package.json` 声明了它、两个源文件 import 它 | **已提交的证据与被提交的树互相矛盾** |

**缺陷 1 的时序才是真正值得记录的**：`cross-tree-paths.test.ts` 这个门在 **19:03:03** 落地，而这两个文件在 **19:10:27** 落地 —— **S6 在门存在的七分钟之后写了它们，没有人重跑门。**

**这与 GAPS.md 里每一条"机制无调用者"的条目是同一个失败**：门存在、门正确、**门没有被跑在合并后的结果上**。一个不被运行的门什么也抓不到。

**处置**：三个缺陷中 #1 和 #2 已修并推送（新 SHA `a85c69f8…`），验证：cross-tree gate **7/7**、`helpers/typecheck.mjs` **PASS**、`s6-ipy15.test.ts` **4/4**。#3 记录而未修（它是再生成的证据产物，属于拥有该门的写入者）。

**给审计者的问题**：本次发布的**第一个** commit（`a476fc13…`）**包含**这三个缺陷。如果你审计的是那个 SHA，你看到的是有缺陷的版本。**请审计 `a85c69f8…`。** 我把这件事写在这里而不是悄悄重推覆盖，因为"发布过的有缺陷版本"本身是审计证据的一部分 —— 而且它证明了本项目最核心的断言：**绿门不等于好东西，而门必须跑在合并后的结果上。**
