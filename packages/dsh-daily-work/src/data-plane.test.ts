...
describe('DataPlaneService: the production consumer the profile mounts', () => {
  ...
  it('records the real artifact-root derivation, including the branch that cannot be taken', async () => {
    ...
    try {
      ...
      //
      // `store.root` is the project's INDEX root -- the bytes live in the mounted
      // provider -- so this assertion pins the derivation, not a directory of
      // objects. The derivation itself is unchanged.
