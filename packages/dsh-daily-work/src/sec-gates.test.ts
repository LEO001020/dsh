...
describe('SEC-06: park invalidates old RPC; reset invalidates old references and tool requests', () => {
  ...
  it('the host service no longer claims a per-await epoch re-check it does not perform', () => {
    ...
    // And the replacement statement names the guard that IS reachable, with its
    // exact limit, so the correction is a pointer rather than a deletion.
    expect(host).toContain('tool-protocol-guards.ts')
    // The root is still persisted as the session-id STRING, and the guard module
    // explains why that alone is not enough — read together, not one excusing the
    // other. The record's own identity field is unchanged by this slice.
    expect(code, 'the root must still be persisted as the session-id string').toMatch(/rootSessionId: input\.root\.session\.header\.id/u)
