...
describe('S15: no NEW cross-tree checkout literal in the source plane', () => {
  ...
  it('every justified entry still exists and still carries its literal, so the allowlist cannot rot', () => {
    ...
    //
    // ONE ENTRY IS EXEMPT FROM THE EXISTENCE HALF, and the exemption is narrow
    // rather than a convenience. `.writer-provision.json` is GENERATED BY
    // PROVISIONING and never committed, so it exists in a writer's worktree and
    // does NOT exist in the integration tree -- which is also where this gate runs.
    // Requiring it to exist makes the gate fail on a correct tree, and both tempting
    // repairs are worse: dropping the entry lets a real provisioned receipt be
    // flagged in every writer worktree, and weakening the arm for all entries
    // removes the rot check entirely.
    //
    // The literal half still applies: IF the file is present it must carry the
    // literal it was allowlisted for, so the entry cannot rot into a blanket
    // exemption.
    const GENERATED_NOT_COMMITTED = new Set(['.writer-provision.json'])
    for (const [rel, reason] of Object.entries(JUSTIFIED_LIVE_LITERALS)) {
      ...
      if (GENERATED_NOT_COMMITTED.has(rel) && !existsSync(abs)) continue
