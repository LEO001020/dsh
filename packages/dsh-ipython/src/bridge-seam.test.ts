...
describe('T7-07 the bridge IS wired into the product (MEASURED — F2 closed)', () => {
  ...
  it('the bridge is CONSTRUCTED by production code, and only by the kernel service', async () => {
    ...
    const PROBE_FILES = new Set([
      ...
      // ADDED DELIBERATELY, which is what this set asks for -- and added
      // independently by two writers, whose comments are merged here rather than
      // one of them dropped. P13's plane probe constructs two bridges to compare
      // the raw-path plane against the unified one, like every probe above: it
      // measures the large-result retention plane on a bridge the probe owns,
      // not a caller the product reaches. It is run by hand
      // (`node --experimental-strip-types src/p13-artifact-plane-probe.ts`), is
      // in no entry point, and no file imports it -- verified, not assumed.
      // Counting it as a production caller would hide exactly what this arm
      // reports.
