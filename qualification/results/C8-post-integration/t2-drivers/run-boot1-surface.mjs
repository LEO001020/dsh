...
const PROFILE = 'daily'
const OUT = `${REPO}/qualification/results/C8-post-integration/boot1-surface.json`
const OVERLAY = `${REPO}/qualification/runners/verify-deliverable-surface.patch.yml`
const TRANSCRIPT = `${REPO}/qualification/results/C8-post-integration/boot1-transcript.txt`
...
if (fatal !== null) {
  ...
  check('CMP-11 the preset root is the INSTALLED profile directory',
    (result.presetRoots ?? []).some(r => String(r.path).toLowerCase().includes(`${HOME.toLowerCase()}/profiles/daily/presets`)),
  ...
  // ── CMP-05 (anchored arm) ───────────────────────────────────────────────
  const ownRoot = (result.presetRoots ?? []).find(r => String(r.path).toLowerCase().includes(`${HOME.toLowerCase()}/profiles/daily/presets`))
...
writeFileSync(`${REPO}/qualification/results/C8-post-integration/boot1-verdict.json`, `${JSON.stringify(verdict, null, 2)}\n`, 'utf8')
