...
//
// It used to be the literal `'D:/DSH/work/dsh-native-daily/qualification/results/M2-scope/boot-probe.json'`.
...
//
// `import.meta.url` is `.../qualification/runners/verify-m2-scope.mjs`, so two levels up is the
...
// describes. `M2_OUT` still overrides for an explicit target.
// ---------------------------------------------------------------------------
...
const OUT = process.env.M2_OUT ?? join(REPO_ROOT, 'qualification/results/M2-scope/boot-probe.json')
