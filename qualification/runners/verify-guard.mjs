...
//
// It used to be the literal `'D:/DSH/work/dsh-native-daily/qualification/results/M9.21-guard-mounted/guard.json'`.
...
//
// `import.meta.url` is `.../qualification/runners/verify-guard.mjs`, so two levels up is the
...
// describes. `GUARD_OUT` still overrides for an explicit target.
// ---------------------------------------------------------------------------
...
const OUT = process.env.GUARD_OUT ?? join(REPO_ROOT, 'qualification/results/M9.21-guard-mounted/guard.json')
