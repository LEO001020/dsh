...
//
// It used to be the literal `'D:/DSH/work/dsh-native-daily/qualification/results/M11-ipython/e2e-tool.json'`.
...
//
// `import.meta.url` is `.../qualification/runners/verify-ipython-e2e.mjs`, so two levels up is the
...
// describes. `IPY_OUT` still overrides for an explicit target.
// ---------------------------------------------------------------------------
...
const OUT = process.env.IPY_OUT ?? join(REPO_ROOT, 'qualification/results/M11-ipython/e2e-tool.json')
